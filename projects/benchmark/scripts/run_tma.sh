...
run_cpu() {
  ...
  for omp in $threads; do
    ...
    OMP_NUM_THREADS=$omp perf stat --topdown -o "$run_dir/perf.omp${omp}.txt" \
  -- "$src_dir/formod" "$active_ctl" data/obs.tab data/atm.tab "$out" \
     TASK time BATCH_SIZE "$cpu_batch_size" > "$log" 2>&1
