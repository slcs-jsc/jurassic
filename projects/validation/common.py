...
def main():
    ...
    manifest = {
        ...
        "lookup_tables": {
            "format": 3,
            "directory_option": "JURASSIC_TBL_DIR or --tbl-dir",
            "files": {gas: {"filename": f"tria_{gas}.nc",
                            "size_bytes": (table_dir / f"tria_{gas}.nc").stat().st_size,
                            "sha256": sha256(table_dir / f"tria_{gas}.nc"),
                            "role": "JURASSIC emissivity lookup table"}
                      for gas in GASES},
        },
        "gases": GASES,
        "cross_sections": {GASES[index]: {"filename": name,
                                           "size_bytes": (xsc_dir / name).stat().st_size,
                                           "sha256": sha256(xsc_dir / name),
                                           "role": "RFM absorption cross section"}
                           for index, name in XSC_FILES.items()} if rfm_bin else None,
        "nu_start": args.nu_start, "nu_end": args.nu_end,
        "channel_count": len(nus),
        "rfm_spectral_step_cm-1": 0.0005,
        "chunk_size": args.chunk_size,
        "spectral_chunk_count": math.ceil(len(nus) / args.chunk_size),
        "parallel_jobs": args.jobs, "geometries": args.geometries,
        "refraction_enabled": True,
        "limb_geometric_tangent_heights_km": LIMB_HEIGHTS,
        "methods": methods,
        "rfm_binary": ({"filename": rfm_bin.name,
                        "size_bytes": rfm_bin.stat().st_size,
                        "sha256": sha256(rfm_bin),
                        "role": "instrumented RFM executable"} if rfm_bin else None),
        "rfm_hitran": ({"filename": rfm_hit.name,
                        "size_bytes": rfm_hit.stat().st_size,
                        "sha256": sha256(rfm_hit),
                        "role": "RFM HITRAN line database"} if rfm_hit else None),
