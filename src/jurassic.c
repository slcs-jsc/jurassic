...
static const double co2230[2001] = { 2.743e-5, 2.8815e-5, 3.027e-5, 3.1798e-5,
...
static const double h2o260[2001] = { .2752, .2732, .2749, .2676, .2667, .2545,
...
double ctmn2(
  ...
  const double b =
    1e6 * LIN(ctmn2_nua[idx], ctmn2_ba[idx], ctmn2_nua[idx + 1],
	      ctmn2_ba[idx + 1], nu);
  const double beta =
    LIN(ctmn2_nua[idx], ctmn2_betaa[idx], ctmn2_nua[idx + 1],
	ctmn2_betaa[idx + 1], nu);
...
double ctmo2(
  ...
  const double b =
    LIN(ctmo2_nua[idx], ctmo2_ba[idx], ctmo2_nua[idx + 1], ctmo2_ba[idx + 1],
	nu);
  const double beta =
    LIN(ctmo2_nua[idx], ctmo2_betaa[idx], ctmo2_nua[idx + 1],
	ctmo2_betaa[idx + 1], nu);
...
void copy_obs(
  ...
  for (int id = 0; id < ctl->nd; id++)
    memcpy(obs_dest->mask[id], obs_src->mask[id],
	   (size_t) obs_src->nr * sizeof(int));
...
#pragma acc parallel loop present(ctl,tbl,atm[0:nbatch],obs[0:nbatch],status[0:nbatch],los[0:nbatch],obs_scratch[0:nbatch])
  for (int ib = 0; ib < nbatch; ib++)
    status[ib] = formod(ctl, tbl, &atm[ib], &obs[ib], &los[ib],
			obs_scratch + ib);
  ...
  for (int ir = 0; ir < obs->nr; ir++) {
	...
	obs->rad[id][ir] += ctl->fov_w[i]
	  * LIN(z[idx], obs_scratch->rad[id][ir0],
		z[idx + 1], obs_scratch->rad[id][ir1],
		zfov);
	obs->tau[id][ir] += ctl->fov_w[i]
	  * LIN(z[idx], obs_scratch->tau[id][ir0],
		z[idx + 1], obs_scratch->tau[id][ir1],
		zfov);
      }
      wsum += ctl->fov_w[i];
    }
...
void intpol_tbl_cga(
  ...
  for (int id = 0; id < ctl->nd; id++) {
	...
	const int ipr = locate_irr(tbl->p[id][ig], tbl->np[id][ig], cgp[ig]);
	...
	if (tbl->nt[id][ig][ipr] < 2 || tbl->nt[id][ig][ipr + 1] < 2
	  ...
	  eps11 = LIN(tbl->t[id][ig][ipr + 1][it1], eps10,
		      tbl->t[id][ig][ipr + 1][it1 + 1], eps11, cgt[ig]);
